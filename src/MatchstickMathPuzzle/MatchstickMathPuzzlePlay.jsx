...
/* -------------------------------------------------------------
   MatchStick (Vertical) – works on mouse & touch
------------------------------------------------------------- */
const MatchStick = ({ match, onDragStart, onDrop }) => {
  ...
  const handleTouchMove = (e) => {
    if (!touchStartPos || !match?.status) return;
    e.preventDefault();
    const touch = e.touches[0];
    ...
    if (Math.abs(dx) > 10 || Math.abs(dy) > 10) {
      ...
      Object.defineProperty(ev, "dataTransfer", {
        value: {
          setData: (type, val) => ((ev)._data = val),
          getData: () => (ev)._data,
        },
        writable: false,
      });
      (ev.dataTransfer).setData("match", "{}");
      onDragStart(ev);
      setTouchStartPos(null);
    }
  };

  const handleTouchEnd = () => setTouchStartPos(null);
  ...
  return (
    <div
      ...
      onDragOver={(e) => !match?.status && e.preventDefault()}
      onDragEnter={() => !match?.status && setHover(true)}
      onDragLeave={() => setHover(false)}
      onDrop={handleDrop}
      onTouchEnd={(e) => !match?.status && handleDrop(e)}
    >
      {match?.status && (
...
/* -------------------------------------------------------------
   MatchStickHorizontal – works on mouse & touch
------------------------------------------------------------- */
  ...
  const handleDragStart = (matchIndex) => (e) => {
    if (matchsticks[matchIndex]?.status) {
      e.dataTransfer.setData(
        "match",
        JSON.stringify({ fromIndex: index, matchIndex })
      );
    } else {
      e.preventDefault();
    }
          ...
          const newValue =
            Object.entries(Patterns).find(
              ([key, pattern]) =>
                ...
                pattern.every(
                  (seg, i) =>
                    seg.id === currentSegments[i].id &&
                    seg.status === currentSegments[i].status
                )
...
export default function MatchstickMathPuzzlePlay() {
  ...
  const Nextindex =
    AllMatchistickPuzzles?.findIndex((d) => d?._id == id) + 1;
  const Nextindex_id =
    AllMatchistickPuzzles[
      AllMatchistickPuzzles?.findIndex((d) => d?._id == id) + 1
    ]?._id;
    ...
    axios
      .post(
        `${import.meta.env.VITE_API_BASE_URL}${import.meta.env
          .VITE_MATCHISTICKSCORESCORE_URL}add`,
        score,
        {
  ...
  const VerifyHandler = () => {
    ...
    axios
      .post(
        `${import.meta.env.VITE_API_BASE_URL}${import.meta.env
          .VITE_MATCHSTICKPUZZLE_URL}resultVerify/${id}`,
        data,
        { headers: { Authorization: usertoken } }
      )
      .then((success) => {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-900 via-purple-900 to-red-900 flex flex-col items-center p-4 sm:p-6 md:p-8">
      ...
      <div className="w-full max-w-4xl flex justify-between items-center text-white text-sm sm:text-base md:text-lg font-semibold mb-4">
        ...
        <p>
          Moves: {showMoves}/{MatchistickPuzzles?.move}
        </p>
      ...
      <div className="w-full max-w-4xl text-center text-white text-sm sm:text-base mb-4">
        <p>
          Drag and drop matchsticks to correct the equation. You have{" "}
          {MatchistickPuzzles?.move} moves.
        </p>
      </div>
      ...
      >
        ...
        <div className="flex gap-4">
          ...
          {Nextindex < AllMatchistickPuzzles?.length && (
            <Link
              to={`/matchstickpuzzle/${Nextindex_id}`}
              onClick={() => {
                setCongrats(false);
                setShowMoves(0);
              }}
      ...
      >
        ...
        <p className="text-sm sm:text-base">
          The equation is incorrect. You have {useMoves - showMoves} moves left.
        </p>
        <div className="flex gap-4">
          <Link
            to={"/matchstickpuzzleLevels"}
