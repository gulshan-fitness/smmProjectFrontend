...
const MatchStick = ({ match, onDragStart, onDrop }) => {
  ...
  const handleTouchStart = (e) => {
    if (match?.status) {
      setTouchData({ startX: e.touches[0].clientX, startY: e.touches[0].clientY });
      e.dataTransfer = { setData: () => {} }; // Fake DnD object
      onDragStart(0)(e); // Reuse logic
    }
  };
  ...
  const handleTouchEnd = (e) => {
    setHover(false);
    if (!match?.status && touchData) {
      e.dataTransfer = { getData: () => JSON.stringify({ matchIndex: 0, fromIndex: 0 }) };
      onDrop(0)(e);
    }
  };
...
// MatchStickHorizontal Component (Horizontal)
const MatchStickHorizontal = ({ match, onDragStart, onDrop }) => {
  ...
  const handleTouchMove = () => setHover(true);
