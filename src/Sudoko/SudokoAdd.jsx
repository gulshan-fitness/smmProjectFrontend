...
export default function AddSudokuPuzzle() {
  ...
  const handleSubmit = async (e) => {
    ...
    axios.post(
      `${import.meta.env.VITE_API_BASE_URL}${import.meta.env.VITE_SUDOKO_URL}add`,
      payload,
      {
        headers: {
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div>
          ...
          <textarea
            ...
            className="w-full bg-black bg-opacity-70 border border-yellow-700 rounded-md px-4 py-3 font-mono text-yellow-300 placeholder-yellow-600 resize-none text-sm sm:text-base
              focus:outline-none focus:ring-2 focus:ring-yellow-500 transition"
            required
          />
        </div>
        ...
        <button
          type="submit"
          className="w-full sm:w-auto px-8 py-3 rounded-md bg-gradient-to-r from-yellow-400 via-yellow-500 to-yellow-600 text-black font-semibold shadow-lg hover:brightness-110 transition-all text-base sm:text-lg"
        >
          Add Puzzle
        </button>
      </form>
