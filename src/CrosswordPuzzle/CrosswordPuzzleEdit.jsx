...
export default function () {
  const { CrosswordPuzzle, CrosswordPuzzleFetch, adminToken, notify } = useContext(Context);
  const { id } = useParams();

  const [title, setTitle] = useState("");
  const [rawGridInput, setRawGridInput] = useState([]);
  const [acrossClues, setAcrossClues] = useState([{ number: "", clue: "", row: "", col: "", answer: "" }]);
  const [downClues, setDownClues] = useState([{ number: "", clue: "", row: "", col: "", answer: "" }]);

  const convertGridToText = (grid) => {
  ...
  useEffect(() => {
    if (!id) return;
    CrosswordPuzzleFetch(id);
  }, [id]);

  useEffect(() => {
    if (!CrosswordPuzzle) return;
    setTitle(CrosswordPuzzle?.title);
    setAcrossClues(CrosswordPuzzle?.clues?.across);
    setDownClues(CrosswordPuzzle?.clues?.down);
    setRawGridInput(convertGridToText(CrosswordPuzzle?.grid));
  }, [CrosswordPuzzle]);
  ...
  const handleAddClue = (type) => {
    ...
    type === "across" ? setAcrossClues([...acrossClues, newClue]) : setDownClues([...downClues, newClue]);
  };
  ...
  const handleSubmit = () => {
    ...
    axios
      .put(
        `${import.meta.env.VITE_API_BASE_URL}${import.meta.env.VITE_CROSSWORDPUZZLE_URL}edit/${id}`,
        newPuzzle,
...
return (
  <div className="min-h-screen bg-[#0C0C0C] p-6 md:p-12 text-[#F5DEB3] font-sans">
    <div className="max-w-6xl mx-auto bg-[#1A1A1A] drop-shadow-[0_4px_6px_rgba(212,175,55,0.7)] p-8 rounded-3xl shadow-xl border border-[#333333]">
